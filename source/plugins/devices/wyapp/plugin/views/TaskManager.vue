				...
				<table class="w-100">
					...
					<tr v-for="task in tasks" :key="task.PID" class="w-100 task">
						<td class="w-50 d-flex">
							<v-img v-if="hasTTY (task)" src="plugins/devices/wyapp/plugin/data/img/icons/task-icon.svg" aria-label="Task" ></v-img>
							<v-img v-else src="plugins/devices/wyapp/plugin/data/img/icons/process-icon.svg" aria-label="Process" ></v-img>
							<h3>{{task.COMMAND}}</h3>
						</td>
