		...
		<v-tooltip top>
			<template v-slot:activator="{ on }">
				<v-btn @click.stop="disconnect" ref="fn" class="icon-btn" v-on="on">
					<img src="plugins/devices/wyapp/data/img/icons/disconnect-icon.svg" :alt="$t('DEVICE_WYAPP_DISCONNECT')" class="s24">
				</v-btn>
			</template>
		...
		<v-tooltip top>
			<template v-slot:activator="{ on }">
				<v-btn @click.stop="disconnectAndReboot" class="icon-btn" v-on="on">
					<img src="plugins/devices/wyapp/data/img/icons/restart-icon.svg" :alt="$t('DEVICE_WYAPP_RESTART')" class="s24">
				</v-btn>
			</template>
		...
		<v-tooltip top>
			<template v-slot:activator="{ on }">
				<v-btn @click.stop="disconnectAndPowerOff" class="icon-btn" v-on="on">
					<img src="plugins/devices/wyapp/data/img/icons/turn-off-icon.svg" :alt="$t('DEVICE_WYAPP_TURNOFF')" class="s24">
				</v-btn>
			</template>
